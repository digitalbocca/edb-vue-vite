...
import { BUTTON_COLOR_CHANGE_TIMEOUT, VARIANTS } from '@/constants'
...
useIntervalFn(
  () => {
    ...
    condition
      ? (coloredButtonsVariant.value = 0)
      : coloredButtonsVariant.value++
  },
  ...
  <div class="flex flex-col">
    <ContainerCard>
      <LeadBlock :title="title">
        <IconHandFinger :width="24" :height="24" :stroke="1.5" />
      </LeadBlock>
    ...
    <ContainerCard>
      ...
      <div class="flex flex-row justify-center flex-wrap gap-2">
        <Button label="Icon Prepend" variant="secondary">
          <IconMoodKidFilled class="w-6 h-6" :stroke="2" />
        </Button>

        <Button label="Icon Append" icon-position="append">
          <IconMoodKidFilled class="w-6 h-6" :stroke="2" />
        </Button>

        <Button icon-position="append" variant="success">
          <IconMoodKidFilled class="w-6 h-6" :stroke="2" />
        </Button>
      </div>
    </ContainerCard>
